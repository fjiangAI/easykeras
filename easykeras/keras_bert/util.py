...
def get_bertmodel(model,poolings=None,vocabs=None,cased=False,output_layer_num=1):
...
def convert_text_to_index(seq_len,tokenizer,texts):
    def _batch_generator(texts):
        tokens, segments = [], []
        def _pad_inputs():
            if seq_len is None:
        ...
        if len(tokens) > 0:
            yield _pad_inputs()
    return  [ result for result in _batch_generator(texts)]

def extract_embeddings_generator_bymodel(seq_len,tokenizer,model,
                                 texts_seq,
                                 poolings=None,
                                 batch_size=4,
                                 cut_embed=False):
    ...
    :param texts_seq: Iterable texts.
    ...
    def _batch_generator(texts):
        ...
        if len(tokens) > 0:
            yield _pad_inputs()
    #处理每一个句子
    def _deal_texts(texts):
        for batch_inputs in _batch_generator(texts):
            ...
            for inputs, output in zip(batch_inputs[0], outputs):
                ...
                yield output
    #使用同一个模型处理多个序列
    for texts in texts_seq:
        yield [embedding for embedding in _deal_texts(texts)]
def extract_embeddings_generator(model,
    ...
    for texts in texts_seq:
        yield [embedding for embedding in _deal_texts(texts)]
def extract_embeddings_bymodel(seq_len,tokenizer,model,texts_seq):
    """Extract embeddings from texts.
    """
    return [embedding for embedding in extract_embeddings_generator_bymodel(
        seq_len,tokenizer,model,texts_seq,
    )]
